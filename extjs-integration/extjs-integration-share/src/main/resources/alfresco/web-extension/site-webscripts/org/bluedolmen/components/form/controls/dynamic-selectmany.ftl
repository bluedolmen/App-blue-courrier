...
(function(BlueDolmen) {
	...
	<#if form.mode == "view">
		<#if field.control.params['loadLabel']??>
			new BlueDolmen.LoadLabel("${fieldHtmlId}-value").setOptions(
			{
	...
	<#else>
		...
		new BlueDolmen.DynamicDropdown("${fieldHtmlId}-entry").setOptions(
		{
			picklistName:"${field.control.params['picklistName']}",
			dependsOn:[
...
<script type="text/javascript">//<![CDATA[
(function(BlueDolmen) {
	BlueDolmen.Dependency.registerHandler("${field.id}","${fieldHtmlId}-entry");
})(window.BlueDolmen = window.BlueDolmen || {});
//]]></script>
