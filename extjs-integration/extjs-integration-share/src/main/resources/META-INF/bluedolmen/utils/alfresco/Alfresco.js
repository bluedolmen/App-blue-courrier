...
Ext.define('Bluedolmen.utils.alfresco.Alfresco', {
	
	alternateClassName: ['Bluedolmen.Alfresco'],
	...
	genericFailureManager : function(response) {
		...
		else if (response && response.serverResponse) {
	...
	// TODO: Stop using alfresco share wrapper and define an independent ws call
	// utility
	getAjaxCallConfig : function(config) {
		...
		var 
			...
			finally_ = config['finally'],
			
			failureCallback_ = function(response) {
				
				try {
					var failureExecResult;
					
					if (onFailure) {
						failureExecResult = onFailure.apply(onFailureScope, arguments);
					}
					
					if (false !== failureExecResult) {
						Bluedolmen.Alfresco.genericFailureManager.apply(this, arguments);
					}
				}
				finally {
					if (Ext.isFunction(finally_)) finally_.call(this);
				}
				
			}, 
			
			successCallback_ = function(response) {
				...
				if (jsonResponse && jsonResponse.status) {
					...
					// Still if we get HTTP success (200), we may
					// have an embedded business error message.
					// We detect it by checking for a status object
					status = jsonResponse.status || {};
					if (status.code || "failure" == status) {
						statusCode = status.code || 500;
						if (Ext.isString(statusCode)) statusCode = Number(statusCode);
					}
				...
				if (statusCode < 200 || statusCode >= 300) {
					// Error
					failureCallback_.call(onFailureScope, response);
					return;
				}
		...
		return Ext.apply(defaultAjaxCallConfig, config);
	...
	jsonRequest : function(config, onSuccess, onFailure, scope) {
		...
		var 
			ajaxCall = Bluedolmen.Alfresco.getAjax(),
	...
	getCurrentUser : function(onPersonAvailable) {
		...
		Bluedolmen.model.PersonFactory.getPerson(currentUserName, onPersonAvailable);
	...
	getCurrentUserHome : function(onUserHomeAvailable) {
		...
		var me = this;
		Bluedolmen.model.UserHome.load(null, {
	...
	splitToTrimmedStringArray : function(str, sep) {
		 ...
		 return Ext.Array.map(str.split(sep), function(elem) {
			 return elem.trim()
			 	.replace(/^[\"\']/, '')
			 	.replace(/[\"\']$/, '')
			 ;
...
Ext.define('Bluedolmen.model.UserHome', {
	extend : 'Ext.data.Model',
	...
	proxy : {
		type : 'rest',
		url : Bluedolmen.Alfresco.getProxyURL() + 'britair/dinamex/userhome',
		reader : 'json'
	}	
