Ext.define('Bluedolmen.utils.alfresco.grid.GridAction', {
		...
		var 
			...
			defaultDefinition =	{
				...
				getClass : function(value, meta, record) {
					...
					var 
						isAvailable = me.isAvailable(record),
						clsClasses = '';
					;
					...
					if (!isAvailable) {
						...
						clsClasses += me.hideClass;
					}
					else {
						if (Ext.isFunction(me.getIconCls)) {
							clsClasses += me.getIconCls(record);
						}
					}
	...
	isBatchAvailable : function(records, context) {
		...
		return Ext.Array.every(records, function(record) {
			return me.isAvailable(record, context) || false;
		});
	...
	/**
	 * This may work on single or iterable data.
	 * 
	...
	performAction : function(/* arguments */) {
		...
		this.fireEvent('actionComplete', 'success', this, this.grid, arguments);
