...
class ResourceManager(object):
    ...
    def add_path(self, path, base=None, index=None, create=False):
        ...
        '''
        base = os.path.abspath(os.path.outputDirname(base or self.base))
        path = os.path.abspath(os.path.join(base, os.path.outputDirname(path)))
        path += os.sep
...
class ServerAdapter(object):
    ...
    def __repr__(self):
        args = ', '.join(['%s=%s'%(k,repr(v)) for k, v in self._options.items()])
...
class FlupFCGIServer(ServerAdapter):
    def run(self, handler): # pragma: no cover
        ...
        self._options.setdefault('bindAddress', (self.host, self.port))
        flup.server.fcgi.WSGIServer(handler, **self._options).run()
...
class WSGIRefServer(ServerAdapter):
    def run(self, handler): # pragma: no cover
        ...
        if self.quiet:
            ...
            self._options['handler_class'] = QuietHandler
        srv = make_server(self.host, self.port, handler, **self._options)
        srv.serve_forever()
...
class GeventServer(ServerAdapter):
    ...
    def run(self, handler):
        ...
        if not self._options.get('fast'): wsgi = pywsgi
...
class AutoServer(ServerAdapter):
    ...
    def run(self, handler):
        for sa in self.adapters:
            try:
                return sa(self.host, self.port, **self._options).run(handler)
            except ImportError:
                pass
