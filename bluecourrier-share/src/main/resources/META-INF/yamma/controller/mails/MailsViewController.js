...
//			},
	...
	onOperation : function(operationKind, context) {
		...
		/*
		 * bpajot (2016013)
		 * The following code is deactivated since it does not provide the
		 * guarantee to release the busy state (loading-mask) ONLY when the
		 * view has been refreshed. The busy-state is released just after the
		 * data-change on the store which may provide incorrect actions to the
		 * user on the node.
		 * <p>
		 * This is obviously suboptimal regarding the cost of time and network-load.
		 */
...
//		}
	...
	onContextChanged : function(context) {
		...
		if (null == context) {
			searchPanel.resetMailsView();
			return;
		}
		
		this.setActiveTab(searchPanel);

		searchPanel.displayContext(context);
