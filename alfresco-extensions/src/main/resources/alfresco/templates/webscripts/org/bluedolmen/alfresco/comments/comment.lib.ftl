...
<#macro renderExtraPermissions node>
<#escape x as jsonUtils.encodeJSONString(x)>
	"inherits" : ${node.inheritsPermissions?string},
    "details" : [
    <#list node.permissions as perm>
    	<#assign def = perm?split(";")>
        {
        	"allowed" : ${("ALLOWED" == def[0])?string},
        	"authority" : "${def[1]}",
            "permission" : "${def[2]}"
        }<#if perm_has_next>,</#if>
    </#list>
...
<#escape x as jsonUtils.encodeJSONString(x)>
{
   ...
   <@renderExtraPermissions node=item.node />
   }
}
</#escape>
</#macro>