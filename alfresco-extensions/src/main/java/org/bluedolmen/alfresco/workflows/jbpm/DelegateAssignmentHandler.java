...
import org.bluedolmen.alfresco.delegates.Delegate;
import org.bluedolmen.alfresco.delegates.DelegateService;
...
public class DelegateAssignmentHandler extends AlfrescoAssignment {

	private static final long serialVersionUID = -2466839506226321964L;
    ...
    protected void initialiseHandler(BeanFactory factory) {
        ...
        delegateService = (DelegateService) factory.getBean("delegateService");
    ...
    private final class AssignmentHelper {
    	...
    	private void _assignActor(String actorId) {
        	...
        	manageDelegateIfNecessary();
    		
    	}
    	
    	private void manageDelegateIfNecessary() {
    		
    		if (delegateService == null) return;
    		
            final Set<Delegate> delegatePersons = delegateService.getDelegatedPersons(assignedActor);
            if (delegatePersons.isEmpty()) return;

            for (final Delegate delegate : delegatePersons) {
            	
            	final String delegateName = delegate.getName();
            	
            	if (delegate.isDefaultAssignee()) {
            		assignedPooledActors.add(assignedActor);
            		assignable.setActorId(delegateName);
            		continue;
            	}
            	
            	assignedPooledActors.add(delegateName);
            	
            }            
    		
    	}
    	
    	private void assignPooledActor() {
